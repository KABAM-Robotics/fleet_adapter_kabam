...
def initialize_fleet(config_yaml, nav_graph_path, node, use_sim_time, server_uri):
    ...
    fleet_handle.add_performable_action("take_photo", _consider)

    # Initialise robot map transforms using utils MapTransform
    maps = config_yaml['maps']
    transforms = {}
    for key, value in maps.items():
        m_transfroms = MapTransform(value['rmf'], value['robot'], value['ori_diff'])
        transforms[key] = m_transfroms.transforms
    ...
    def _updater_inserter(cmd_handle, update_handle):
        ...
        def _action_executor(
            ...
            with cmd_handle._lock:
                ...
                cmd_handle.node.get_logger().info("Action Executor:")
                cmd_handle.node.get_logger().info(f"{category} | {description}")
                if category == "take_photo":
                    cmd_handle.node.get_logger().info("Take the photo...")
                    cmd_handle.api.trigger_image_capture()
                    execution.finished()
    ...
    api = RobotAPI(
        node.get_logger(),
        fleet_config['fleet_manager']['robot_api_prefix'],
        fleet_config['fleet_manager']['robot_api_user'],
        fleet_config['fleet_manager']['robot_api_password'],
        fleet_config['fleet_manager']['cloud_api_prefix'],
        fleet_config['fleet_manager']['cloud_api_user'],
        fleet_config['fleet_manager']['cloud_api_password'])
            ...
            for robot_name in list(missing_robots.keys()):
                ...
                if len(position) > 2:
                    ...
                    if (initial_waypoint is not None) and\
                        ...
                        initial_waypoint_index = None
                        try:
                            initial_waypoint_index = nav_graph.find_waypoint(
                                initial_waypoint).index
                        except AttributeError:
                            node.get_logger().error("Incorrect start waypoint" +
                                                    " set in CoLab_config.yaml. " +
                                                    "Exiting...")
                            sys.exit(1)

                        assert initial_waypoint_index is not None
                        ...
                        node.get_logger().info(
                            "Running compute_plan_starts for robot: "
                            f"{robot_name}")
                    ...
                    with open(nav_graph_path, 'r') as file:
                        ...
                        try:
                            ...
                            node.get_logger().error("Different map_name in " +
                                                    "nav_graph.yaml. Exiting...")
                            sys.exit(1)
...
def main(argv=sys.argv):
    ...
    node.get_logger().info("Starting [fleet_adapter_kabam]...")
